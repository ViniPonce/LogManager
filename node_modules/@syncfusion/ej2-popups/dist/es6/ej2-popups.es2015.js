...
let Tooltip = class Tooltip extends Component {
    ...
    getScalingFactor() {
        const xScalingFactor = this.element.getBoundingClientRect().width / this.element.offsetWidth;
        const yScalingFactor = this.element.getBoundingClientRect().height / this.element.offsetHeight;
    ...
    calculateTooltipOffset(position, xScalingFactor = 1, yScalingFactor = 1) {
        ...
        const tooltipEleWidth = Math.round(tooltipEleRect.width);
        const tooltipEleHeight = Math.round(tooltipEleRect.height);
        const arrowEle = select('.' + ARROW_TIP, this.tooltipEle);
        if (arrowEle) {
            arrowEleRect = arrowEle.getBoundingClientRect();
        }
        const tipWidth = arrowEle ? Math.round(arrowEleRect.width) : 0;
        const tipHeight = arrowEle ? Math.round(arrowEleRect.height) : 0;
        let tipAdjust = (this.showTipPointer ? SHOW_POINTER_TIP_GAP : HIDE_POINTER_TIP_GAP);
        const tipHeightAdjust = (tipHeight / 2) + POINTER_ADJUST + (tooltipEleHeight - (this.tooltipEle.clientHeight * yScalingFactor));
        const tipWidthAdjust = (tipWidth / 2) + POINTER_ADJUST + (tooltipEleWidth - (this.tooltipEle.clientWidth * xScalingFactor));
        if (this.mouseTrail) {
